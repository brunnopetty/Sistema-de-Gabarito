...
while True:
    ...
    try:
        ...
        #MOSTRANDO OS VALORES DAS MARCAÇÕES
            ...
            imRawDrawing = utlis.showAnswers(imRawDrawing, myIndex, grading, respostas, questoes, escolhas)
            invMatrix = cv2.getPerspectiveTransform(pt2, pt1)
            imgInvWarp = cv2.warpPerspective(imRawDrawing, invMatrix, (widthImg, heightImg))

            imgRawGrade = np.zeros_like(imgGradeDisplay)
            cv2.putText(imgRawGrade,str(int(nota))+"%",(80,100),cv2.FONT_HERSHEY_COMPLEX,3,(0,255,255),4)
    ...
    cv2.imshow("Resultado Final", imgFinal)
    cv2.imshow("Imagens Empilhadas",imgStacked)
    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite("ResultadoFinal.jpg", imgFinal)
        cv2.waitKey(300)